...
(function () {
  ...
  async function pageBank(view) {
    ...
    async function laad() {
      ...
      const losseRows = losse.length ? losse.map((t) => `<tr>
        ...
        : `<tr><td colspan="6" class="empty">\u2713 Elke boeking die over je bank loopt hoort bij een bankregel.</td></tr>`;
      ...
      view.innerHTML =
        ...
        <div class="tabs" style="margin-bottom:16px;display:flex;gap:8px">${tabs.map((t) => `<button data-tab="${t}" class="${filter === t ? 'active' : ''}">${t}</button>`).join('')}</div>
         ...
         <div class="card" style="margin-bottom:24px"><div class="card-head"><span>Boekingen zonder bankregel${losse.length ? ' (' + losse.length + ')' : ''}</span></div>
           <div class="mut" style="padding:12px 20px 0;font-size:12px;line-height:1.5">Volgens het grootboek ging hier geld van je bankrekening af of erop, terwijl er geen bankregel bij hoort \u2014 inclusief <b style="color:var(--inkdim)">overboekingen</b> en andere memoriaalposten vanaf je eerste ingelezen bankregel. Dat is een <b style="color:var(--inkdim)">dubbele boeking</b>, of een boeking die bij een nog <b style="color:var(--inkdim)">open</b> bankregel hoort. Let op het bedrag: staat hier een boeking van \u20AC 7,45 terwijl de open bankregel \u20AC 9,02 is, dan is dat de factuur die ooit met het verkeerde BTW-regime is gesplitst \u2014 verwijder hem hier en boek de bankregel opnieuw met <b style="color:var(--inkdim)">Boek</b>. De boekhouding van je accountant (XAF- of jaarrekening-import) van vóór je eerste bankregel blijft hier buiten: daarvoor is geen afschrift ingelezen, dus valt er niets te koppelen. Telt het totaal hieronder precies op tot het verschil in de aansluiting hierboven, dan heb je de oorzaak te pakken.</div>
           ...
           <tbody>${losseRows}</tbody>${losse.length ? `<tfoot><tr><td></td><td style="font-weight:500;color:var(--inkdim)">Samen</td><td class="num" style="font-weight:600">${euro(Math.abs(round2(losse.reduce((sm, t) => sm + t.bankbedrag, 0))))}</td><td colspan="3" class="mut" style="font-size:12px">netto ${round2(losse.reduce((sm, t) => sm + t.bankbedrag, 0)) >= 0 ? 'van de bank af' : 'op de bank bij'} volgens het grootboek</td></tr></tfoot>` : ''}</table></div>
  ...
  async function privMaand(view) {
    ...
    view.querySelectorAll('[data-maand]').forEach((cell) => cell.onclick = () => openPriveMaandDetail(cell.dataset.cat, cell.dataset.catnaam, cell.dataset.maand, () => privMaand(view)));
    document.getElementById('pjaar').onchange = (e) => { const j = Number(e.target.value); if (j >= 2000 && j <= 2100) { priveJaar = j; laad(); } };
  }

  async function openPriveMaandDetail(catId, catNaam, maand, onChange) {
    ...
    const close = () => ov.remove();
    ov.addEventListener('click', (e) => { if (e.target === ov) close(); });
    ov.innerHTML = `<div class="modal lg"><div class="modal-head"><h2>${esc(titel)}</h2><button class="x">✕</button></div><div class="modal-body" id="md"><div class="mut">Laden…</div></div></div>`;
    ov.querySelector('.x').onclick = close;
    const md = ov.querySelector('#md');
    let rek = [], cats = [];
    try { [rek, cats] = await Promise.all([api('prive_rekeningen'), api('prive_categorieen')]); } catch (e) { /* edit lukt dan niet, tonen kan wel */ }
    ...
    async function renderInhoud() {
      ...
      }).join('') : `<tr><td colspan="${gedeeld ? 7 : 6}" class="empty">Geen transacties.</td></tr>`;
      md.innerHTML = `
        <div style="overflow-x:auto;min-width:0"><table class="compact">
          <thead><tr><th>Datum</th><th>Rekening</th><th>Tegenpartij</th><th>Omschrijving</th><th class="r">Bedrag</th>${gedeeld ? '<th class="r">Jouw deel</th>' : ''}<th></th></tr></thead>
          <tbody>${rows}</tbody>
          <tfoot><tr><td colspan="4" style="font-weight:600;color:var(--inkdim)">Totaal (${tx.length})</td><td class="num" style="font-weight:600">${euro(totVol)}</td>${gedeeld ? `<td class="num" style="font-weight:700;color:var(--ink)">${euro(totDeel)}</td>` : ''}<td></td></tr></tfoot>
        </table></div>
      ...
      md.querySelectorAll('[data-edit-tx]').forEach((b) => b.onclick = () => {
        const t = tx.find((x) => x.id === Number(b.dataset.editTx));
        if (t) openPriveTransactie(t, rek, cats, () => { renderInhoud(); if (onChange) onChange(); });
      });
    }
    await renderInhoud();
  }
  ...
  async function privTransacties(view) {
    ...
    view.innerHTML = pageHead('Transacties', 'Al je privéboekingen — wijs categorieën toe om je uitgaven te volgen.', `<button class="btn btn-brand" id="nieuwTx">+ Transactie</button>`) + `
      <div class="tabs" style="margin-bottom:16px;display:flex;gap:8px;flex-wrap:wrap">${TABS.map(([k, l, n]) => `<button data-tab="${k}" class="${priveTxTab === k && !priveTxFilter.categorie ? 'active' : ''}">${l} <span class="badge">${n}</span></button>`).join('')}</div>
      <div class="page-actions" style="margin-bottom:12px;flex-wrap:wrap;gap:8px">
        ...
        <select id="fCat" style="width:auto"><option value="">Alle categorieën</option>${cats.map((c) => `<option value="${c.id}" ${String(priveTxFilter.categorie) === String(c.id) ? 'selected' : ''}>${esc(c.naam)}</option>`).join('')}</select>
      ...
      </tbody></table></div></div>${paginatie}${faqBlock('privetx')}`;
    ...
    view.querySelectorAll('[data-sug]').forEach((b) => b.onclick = async () => { try { await api('prive_transactie_categorie', { id: Number(b.dataset.sug), categorieId: Number(b.dataset.sugcat), onthoud: false }, 'POST'); toast('Voorstel toegepast ✓'); laad(); } catch (e) { toast(e.message, 'error'); } });
    ...
    view.innerHTML = pageHead('Categorieën', 'Groepeer je uitgaven en inkomsten.', `<button class="btn btn-brand" id="nieuwCat">+ Categorie</button>`) +
      `<div class="help" style="margin-bottom:16px">Categorieën bepalen hoe je uitgaven en inkomsten worden gegroepeerd. Wijs ze toe bij Transacties; met "onthoud" leert de app ze automatisch toe te kennen bij een volgende import.</div>
        ...
        <div class="card" style="margin-top:24px"><div class="card-head"><span>Automatische regels</span><div style="display:flex;gap:8px"><button class="btn btn-ghost" id="pasToe">Regels toepassen</button><button class="btn btn-brand" id="nieuwRegel">+ Regel</button></div></div>
          <div class="mut" style="padding:12px 20px 0;font-size:12px;line-height:1.5">Een regel koppelt een <b style="color:var(--inkdim)">zoekterm</b> (die in de tegenpartij of omschrijving voorkomt) aan een categorie. Bij import wordt er <b style="color:var(--inkdim)">nooit automatisch geboekt</b> — je krijgt per transactie alleen een <b style="color:var(--inkdim)">voorstel</b> dat je zelf toepast. Regels ontstaan alleen als je bij een transactie "onthoud" aanvinkt. Met <b style="color:var(--inkdim)">Regels toepassen</b> kun je desgewenst alle voorstellen in één keer op de ongecategoriseerde transacties toepassen (handmatige actie).</div>
  ...
  const FAQ = {
    ...
    inkoop: { q: 'Hoe boek ik een inkoopfactuur?', a: 'Ga naar <b>Facturen invoeren → Inkoop</b> en upload de PDF. De AI leest bedrag, BTW en datum. Kies de <b>kostenrekening</b> (waar gaat het over) en de <b>betaalrekening</b> (je bank). De app splitst automatisch bedrag excl. BTW + de voorbelasting op <code>{{btwVoor}}</code>.' },
    buitenland: { q: 'Hoe boek ik een buitenlandse leverancier (Anthropic, Hosting.com) zonder NL-BTW?', a: 'Leg de leverancier vast (Bank-pagina → Leveranciers) met het juiste <b>BTW-regime</b>: <b>geen</b> voor niet-EU (bv. VS — geen BTW, telt niet mee in de aangifte) of <b>verlegd</b> voor EU-diensten (de BTW wordt naar jou verlegd). Bij "verlegd" boekt de app de BTW zowel als verschuldigd (<code>{{btwVersch}}</code>) als voorbelasting (<code>{{btwVoor}}</code>) — saldo € 0 — en zet het in rubriek 4b. Kies bij de boeking hetzelfde bij "BTW".' },
    verkoop: { q: 'Hoe boek ik een verkoopfactuur?', a: 'Ga naar <b>Facturen invoeren → Verkoop</b>, vul het bedrag excl. BTW en het tarief (21% of 9%) in. De app boekt de omzet als opbrengst en de verschuldigde BTW op <code>{{btwVersch}}</code>.' },
    ...
    btwwerking: { q: 'Hoe werkt de BTW in deze app?', a: 'Je verkopen leveren <b>verschuldigde BTW</b> op (<code>{{btwVersch}}</code>), je inkopen leveren <b>voorbelasting</b> op (<code>{{btwVoor}}</code>). Per kwartaal is het saldo ({{btwVersch}} − {{btwVoor}}) je aangifte: positief = betalen, negatief = terugkrijgen.' },
    btwafdracht: { q: 'Hoe boek ik de BTW-afdracht of -teruggaaf?', a: 'Dat gaat in <b>twee stappen</b>, en dat is met opzet.<br><b>1.</b> Ga naar de <b>BTW</b>-pagina, kies het kwartaal en klik <b>Afdracht boeken</b> (of Teruggaaf). De app vult een memoriaal voor dat al je BTW-rekeningen (o.a. <code>{{btwVersch}}</code> verschuldigd en <code>{{btwVoor}}</code> voorbelasting) naar € 0 boekt, met als datum de <b>laatste dag van het kwartaal zelf</b>. De tegenkant is je tussenrekening <code>{{btwAfreken}}</code>.<br><b>2.</b> Zodra je betaalt of de teruggaaf ontvangt, staat die regel in je bankafschrift. Ga naar <b>Bank</b>, klik bij die regel op <b>overboeking</b> en kies <code>{{btwAfreken}}</code> als tegenrekening. Daarmee loopt ook de tussenrekening weer naar 0.<br>Waarom niet in één keer tegen de bank? Omdat de betaling meestal in het <b>volgende kwartaal</b> valt. Die mutatie zou dan meetellen in de aangifte van dát kwartaal, terwijl er geen factuur tegenover staat. Via de tussenrekening sluit elk kwartaal netjes op nul. Stel die rekening in bij <b>Instellingen → BTW-rekeningen</b>; zonder tussenrekening boekt stap 1 direct tegen de bank en loop je precies tegen dat probleem aan.' },
    ...
    priveCat: { q: 'Hoe zie ik waar mijn geld heen gaat?', a: 'Geef transacties een <b>categorie</b> (tabblad Transacties). Met "onthoud" aan onthoudt de app die tegenpartij als <b>regel</b>, zodat je bij een volgende import een <b>voorstel</b> krijgt (nooit automatisch geboekt — je klikt het zelf toe). Op het <b>Overzicht</b> zie je je uitgaven per categorie.' },
    ...
    priveLening: { q: 'Hoe boek ik de storting van een ontvangen lening (bv. € 10.000)?', a: 'Een lening is <b>geen inkomen</b> — het is geld dat je moet terugbetalen. Twee stappen:<br>• De <b>+ € 10.000</b> die op je bank binnenkomt: geef de categorie <b>Ontvangen / verstrekte lening</b> (neutraal), zodat het niet als inkomst telt.<br>• Leg op de tab <b>Te ontvangen / betalen</b> een <b>schuld</b> van € 10.000 vast (bv. "Lening [naam]"). Zo daalt je vermogen met wat je nog moet terugbetalen; per saldo maakt de lening je niet rijker.<br>Aflossingen boek je later ook <b>neutraal</b> (Ontvangen/verstrekte lening) en je verlaagt de schuldpost. De <b>rente</b> die je betaalt is wél een uitgave — geef die een categorie <b>Rente</b>.' },
    priveKinderenCash: { q: 'Mijn kind geeft mij contant geld en ik stort dat bedrag op hún rekening — hoe boek ik dat?', a: 'Vanuit jóuw kant is dit een <b>overboeking tussen je eigen bank en je contant-pot</b>: er gaat een bedrag van je bankrekening af (naar de rekening van je kind) en je ontvangt datzelfde bedrag <b>contant</b>. De rekening van je kind staat niet in je boekhouding, dus die negeer je. Klik bij de <b>− regel</b> op je bank op <b>overboeking → "Tegenkant erbij boeken"</b> en kies je <b>contant-pot</b> (kas). De app boekt dan hetzelfde bedrag <b>+</b> op je kas, zodat je kassaldo klopt. Geen uitgave of inkomst, alleen een verschuiving. Heb je nog geen kas? Maak er een aan met soort <b>Contant</b> op de tab Rekeningen.' },
    priveBelastingGezamenlijk: { q: 'Een belastingteruggaaf die 100% van mij is, kwam op de gezamenlijke rekening en heb ik doorgeboekt naar privé — hoe tag ik de 3 boekingen?', a: 'Tag de teruggaaf op de rekening waar hij 100% van jou is (je <b>privérekening</b>), en laat de gezamenlijke rekening alleen doorlopen:<br>• Gezamenlijk <b>+ bedrag</b> (binnenkomst) → <b>Overboeking eigen rekening</b> (neutraal).<br>• Gezamenlijk <b>− bedrag</b> (doorboeking naar privé) → <b>Overboeking eigen rekening</b> (neutraal).<br>• Privé <b>+ bedrag</b> (binnenkomst) → categorie <b>Belasting</b>.<br>De twee regels op de gezamenlijke rekening salderen naar € 0, dus de 50%-weging speelt niet mee; op je privérekening telt de teruggaaf voor het <b>volle bedrag</b>. Zo klopt ook je vermogen (de hele teruggaaf zit nu bij jou).' },
  ...
  const FAQ_PAGES = {
    ...
    privetx: ['priveOverboeking', 'priveKinderenCash', 'priveBelastingGezamenlijk', 'priveLening', 'priveNeutraal', 'priveCat'],
  };
  /* De BTW-rekeningen zijn instelbaar, dus noem ze nooit hard in een FAQ-antwoord.
     Schrijf {{btwVoor}} / {{btwVersch}} / {{btwAfreken}} en vul ze hier in. */
  function faqVul(tekst) {
    const afr = (state.settings && state.settings.btwAfrekenrekening) || '';
    return String(tekst)
      .replace(/\{\{btwVoor\}\}/g, esc(btwVoorRek()))
      .replace(/\{\{btwVersch\}\}/g, esc(btwVerschRek()))
      .replace(/\{\{btwAfreken\}\}/g, afr ? esc(afr) : 'je tussenrekening');
  }
  function faqBlock(pageKey) {
    ...
    return `<div class="card" style="margin-top:24px"><div class="card-head">❓ Hoe boek ik… — veelgestelde vragen</div>
      <div class="card-body" style="display:flex;flex-direction:column;gap:6px">
        ${items.map((f) => `<details class="faq"><summary>${esc(f.q)}</summary><div class="faq-a">${faqVul(f.a)}</div></details>`).join('')}
      </div></div>`;
  }
